...
def module_setup(request, device, data_dir, platform_data_dir, app_dir, artifact_dir):
    def module_teardown():
        ...
        device.run_ssh('cp -r /var/snap/kimai/current/config/.env {0}/config.current'.format(TMP_DIR), throw=False)
...
def test_storage_change_event(device):
    device.run_ssh('snap run kimai.storage-change > {0}/storage-change.log'.format(TMP_DIR))
